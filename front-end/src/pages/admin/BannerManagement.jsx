    ...
    <div className="space-y-6 text-left relative">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-xs">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Lập lịch trình và thiết kế băng rôn, trình chiếu quảng cáo toàn trang</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200 p-8 shadow-xs">
          ...
          <h3 className="text-lg font-bold text-gray-800">Không tìm thấy banner</h3>
          <p className="text-sm text-gray-400 mt-1">Vui lòng thay đổi bộ lọc hoặc thêm một banner mới.</p>
        </div>
      ) : (
        // Banners List
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="divide-y divide-gray-100 text-sm font-medium text-gray-700">
                {banners.map((banner) => {
                  const isActivePosition = bannerPositions.find(p => p.value === banner.position)?.label || banner.position;
                  const now = new Date();
                  ...
                  return (
                    <tr key={banner.id} className="hover:bg-slate-50/50 transition duration-150">
                      ...
                      <td className="px-6 py-4 max-w-[200px]">
                        <div className="font-bold text-gray-900 truncate">{banner.title}</div>
                        <div className="text-xs text-gray-400 truncate mt-0.5">{banner.subtitle || "Không có phụ đề"}</div>
                      ...
                      {/* Position & Order */}
                      <td className="px-6 py-4">
                        <div className="text-xs font-semibold text-gray-600">{isActivePosition}</div>
                        <div className="text-[11px] text-gray-400 mt-0.5">Thứ tự: {banner.sortOrder || 0}</div>
                      </td>

                      {/* Timeframe */}
                      <td className="px-6 py-4 text-xs space-y-1">
                        <div><span className="text-gray-400 font-semibold">Từ:</span> {formatDateTime(banner.startDate)}</div>
                        <div><span className="text-gray-400 font-semibold">Đến:</span> {formatDateTime(banner.endDate)}</div>
                        {!isDateActive && (
                      ...
                      <td className="px-6 py-4 text-center">
                        ...
                        >
                          ...
                          <div className={`w-11 h-6 bg-gray-200 rounded-full transition-colors duration-200 relative
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 relative text-left">
            ...
            <div className="flex justify-between items-center border-b border-gray-100 pb-3.5 mb-5">
              <h2 className="text-lg font-black text-gray-900">
                {modalMode === "add" ? "Thêm Banner Quảng cáo mới" : "Chỉnh sửa Banner"}
              </h2>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Tiêu đề Banner *</label>
                  <input
                    type="text"
                    required
                    value={formTitle}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Phụ đề (Subtitle)</label>
                  <input
                    type="text"
                    value={formSubtitle}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-500 uppercase block">Ảnh Desktop Banner *</label>
                  <div className="relative aspect-[16/9] w-full rounded-xl bg-gray-50 border border-gray-150 flex items-center justify-center overflow-hidden">
                    {formImageUrl ? (
                      <>
                        <img src={formImageUrl} alt="Desktop Preview" className="w-full h-full object-cover" />
                        <button
                          type="button"
                      ...
                      <label className="flex flex-col items-center justify-center p-4 text-center cursor-pointer w-full h-full">
                        {uploadingDesktop ? (
                          ...
                          <>
                            ...
                            <span className="text-xs text-gray-500 font-semibold">Tải ảnh Desktop</span>
                            <span className="text-[10px] text-gray-400 mt-0.5">Tỷ lệ rộng (16:9 / 21:9)</span>
                          </>
                        )}
                        <input
                          type="file"
                ...
                {/* Mobile Image */}
                <div className="space-y-2">
                  <label className="text-xs font-bold text-gray-500 uppercase block">Ảnh Mobile Banner</label>
                  <div className="relative aspect-[16/9] md:aspect-square md:max-h-[145px] w-full rounded-xl bg-gray-50 border border-gray-150 flex items-center justify-center overflow-hidden">
                    {formMobileImageUrl ? (
                      <>
                        <img src={formMobileImageUrl} alt="Mobile Preview" className="w-full h-full object-cover" />
                        <button
                          type="button"
                      ...
                      <label className="flex flex-col items-center justify-center p-4 text-center cursor-pointer w-full h-full">
                        {uploadingMobile ? (
                          ...
                          <>
                            <Upload className="h-7 w-7 text-gray-300 mb-1" />
                            <span className="text-xs text-gray-500 font-semibold">Tải ảnh Mobile</span>
                            <span className="text-[10px] text-gray-400 mt-0.5">Tỷ lệ đứng hoặc vuông (1:1 / 4:5)</span>
                          </>
                        )}
                        <input
                          type="file"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Vị trí hiển thị *</label>
                  <select
                    value={formPosition}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Chữ nút (Button text)</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Thứ tự hiển thị (sortOrder)</label>
                  <input
                    type="number"
                    min="0"
                    value={formSortOrder}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-gray-50">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Kiểu liên kết *</label>
                  <select
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Thời gian chạy *</label>
                  <input
                    type="datetime-local"
                    required
                    value={formStartDate}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-500 uppercase">Thời gian kết thúc *</label>
                  <input
                    type="datetime-local"
                    required
                    value={formEndDate}
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                />
                <label htmlFor="formIsActive" className="text-sm font-bold text-gray-700 cursor-pointer selection:bg-transparent">

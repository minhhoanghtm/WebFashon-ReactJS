...
const LookbookManagement = () => {
  ...
  const handleOpenEdit = async (lookbookObj) => {
    try {
      ...
      if (page.publishedAt) {
        setFormPublishedAt(new Date(page.publishedAt).toISOString().slice(0, 16));
      } else {
        setFormPublishedAt("");
      }
    ...
    <div className="space-y-6 text-left relative text-slate-800">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-xs">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Thiết kế giao diện bộ sưu tập tạp chí, ảnh bìa, quote nghệ thuật, gallery sản phẩm nổi bật.
          </p>
        </div>
        <button
          onClick={handleOpenAdd}
          className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-black text-sm font-bold rounded-xl shadow-md hover:scale-102 active:scale-98 transition cursor-pointer shrink-0"
        >
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} className="bg-white p-5 rounded-2xl border border-gray-100 animate-pulse flex items-center justify-between gap-6">
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-200 p-8 shadow-xs">
          ...
          <h3 className="text-lg font-bold text-gray-800">Không tìm thấy bộ sưu tập nào</h3>
          <p className="text-sm text-gray-400 mt-1">Nhấn nút bên trên để tạo một bộ sưu tập Lookbook mới.</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {lookbooks.map((p) => {
                  const statusLabel = pageStatuses.find((s) => s.value === p.status)?.label || p.status;
                  ...
                  return (
                    <tr key={p._id} className="hover:bg-gray-50/80 transition-colors">
                      <td className="px-6 py-4">
                        <div className="w-14 h-10 bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
                          {p.thumbnailUrl ? (
                            ...
                            <span className="text-gray-300 text-[10px] flex items-center justify-center h-full">Không ảnh</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 font-mono text-gray-505">{p.displayOrder}</td>
                      <td className="px-6 py-4 font-bold text-gray-750">{p.viewCount || 0}</td>
          ...
          {pagination.totalPages > 1 && (
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50 px-6 py-4 border-t border-gray-100">
              <span className="text-xs text-gray-500 font-medium">
                Hiển thị trang {pagination.page} trên tổng {pagination.totalPages} ({pagination.totalItems} bộ sưu tập)
              </span>
          ...
          <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden flex flex-col my-8 max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              <div>
                <h3 className="text-lg font-black text-gray-900">
                  {modalMode === "add" ? "Thêm Lookbook mới" : "Chỉnh sửa Lookbook"}
                </h3>
                <p className="text-xs text-gray-500 mt-0.5">Cấu hình các thông tin và bố cục khối tạp chí</p>
              </div>
              <button
                ...
                <div className="space-y-6">
                  <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="w-full md:w-1/3 space-y-2 text-left shrink-0">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider block">Ảnh bìa bộ sưu tập</label>
                      <div className="relative aspect-[2/3] w-full rounded-2xl bg-gray-50 border border-gray-200 hover:border-indigo-300 transition flex flex-col items-center justify-center overflow-hidden group">
                        {formThumbnailUrl ? (
                          <>
                            <img src={formThumbnailUrl} alt="Cover Preview" className="w-full h-full object-cover" />
                          ...
                          <label className="flex flex-col items-center justify-center p-6 text-center cursor-pointer w-full h-full">
                            {uploadingThumbnail ? (
                              ...
                              <>
                                ...
                                <span className="text-xs text-gray-600 font-bold">Tải ảnh bìa</span>
                                <span className="text-[10px] text-gray-400 mt-1">Tỉ lệ dọc (2:3) khuyên dùng</span>
                              </>
                            )}
                            <input
                              type="file"
                    ...
                    <div className="flex-1 space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5 text-left">
                          <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Tiêu đề bộ sưu tập *</label>
                          <input
                            type="text"
                            required
                            value={formTitle}
                        ...
                        {/* Status */}
                        <div className="space-y-1.5 text-left">
                          <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Trạng thái xuất bản</label>
                          <select
                            value={formStatus}
                        ...
                        {/* Display Order */}
                        <div className="space-y-1.5 text-left">
                          <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Thứ tự hiển thị (displayOrder)</label>
                          <input
                            type="number"
                        ...
                        <div className="space-y-1.5 text-left md:col-span-2">
                          <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Thời gian xuất bản</label>
                          <input
                            type="datetime-local"
                      ...
                      {/* Excerpt */}
                      <div className="space-y-1.5 text-left">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Mô tả ngắn / Slogan bộ sưu tập</label>
                        <textarea
                          value={formExcerpt}
                  ...
                  <div className="space-y-4 pt-8 border-t border-gray-100 text-left">
                    <div className="flex items-center gap-2 text-indigo-700 font-black">
                      ...
                      <h4 className="text-xs uppercase tracking-wider">Cấu hình SEO cho trang</h4>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-1.5">
                        <label className="text-xs font-bold text-gray-550 uppercase tracking-wider">Tiêu đề SEO</label>
                        <input
                          type="text"
                          value={formSeoTitle}
                      ...
                      <div className="space-y-1.5">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Từ khóa SEO</label>
                        <input
                          type="text"
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Mô tả SEO</label>
                      <textarea
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-gray-100 flex justify-end gap-3 shrink-0 select-none">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-5 py-2 bg-indigo-600 hover:bg-indigo-200 text-black text-xs font-bold rounded-xl shadow-md disabled:opacity-50 disabled:cursor-not-allowed transition cursor-pointer"
              >
